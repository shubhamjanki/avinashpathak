...
export const Route = createFileRoute("/appointment")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Schedule a consultation at the chambers of Advocate Avinash Pathak. Legal consultation, community-rate sessions, and writer engagements." },
...
const TIERS = [
  {
    ...
    usd: "US$ 10",
    per: "per session",
    body: "A concessional rate reserved for students, volunteers and clients of limited means — the same preparation, at a community price.",
...
function Appointment() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        err?.message || "Failed to submit appointment request. Please check your details and try again."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="md:col-span-7">
            {sent ? (
              <div className="border border-gold/50 bg-navy p-8">
                ...
                <div className="mt-6 border-t border-border/40 pt-4 text-xs text-paper/70">
                  A confirmation email has been dispatched to your provided address.
                </div>
              </div>
            ) : (
              <form
              ...
              >
                {error && (
                  <div className="border border-red-500/40 bg-red-950/40 p-4 text-xs text-red-200">
                    {error}
                  </div>
                )}
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <div>
                    ...
                    <input name="time" type="time" className="mt-2 w-full border border-border bg-navy px-4 py-3 text-sm text-paper focus:border-gold focus:outline-none" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input name="country" type="text" placeholder="e.g. India · IST or United Kingdom · GMT+1" className="mt-2 w-full border border-border bg-navy px-4 py-3 text-sm text-paper placeholder:text-muted-foreground focus:border-gold focus:outline-none" />
                </div>
                <div>
                  ...
                  <select name="tier" className="mt-2 w-full border border-border bg-navy px-4 py-3 text-sm text-paper focus:border-gold focus:outline-none">
                    ...
                    <option>Community rate — ₹844</option>
                ...
                <div>
                  ...
                  <textarea name="note" rows={5} required placeholder="Describe your matter in plain language…" className="mt-2 w-full border border-border bg-navy px-4 py-3 text-sm text-paper placeholder:text-muted-foreground focus:border-gold focus:outline-none" />
                </div>
                <button
                  type="submit"
                  disabled={loading}
                  className="mt-2 inline-flex items-center justify-center bg-paper px-6 py-3 text-sm font-medium text-navy transition-colors hover:bg-gold disabled:opacity-50 disabled:cursor-not-allowed"
                >
